...
export function renderApp(state) {
  ...
  app.innerHTML = `<div class="app-shell">${renderSidebar(state.selectedMarket)}<main class="main"><header class="topbar"><div><p class="breadcrumb">市场总览 / 农业品种 / 全国分布</p><h1>全国农业期货分布图</h1></div><div class="toolbar"><button id="route-toggle" class="tool ${state.showRoutes ? 'on' : ''}">${state.showRoutes ? '产区关联' : '显示关联'}</button><button id="motion-toggle" class="tool">${state.motion ? '关闭动态' : '开启动态'}</button></div></header><section class="metric-grid"><div><span>已纳入交易所</span><b>3</b></div><div><span>农业期货目录</span><b>23</b></div><div><span>已接入行情品种</span><b class="good">2</b></div><div><span>主产区锚点</span><b>4</b></div></section><section class="workspace">${renderMapView(state)}${renderMarketDrawer(state.selectedMarket)}</section>${renderStatusPanels()}</main></div>`;
}
